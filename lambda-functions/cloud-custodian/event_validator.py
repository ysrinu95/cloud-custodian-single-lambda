...
class EventValidator:
    ...
    def _validate_cloudtrail_event(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Extract userIdentity first (needed for both account and creator)
        ...
        # Extract creator/user information from userIdentity
        creator_name = None
        if user_identity:
            # Try userName first (IAM users)
            creator_name = user_identity.get('userName')
            
            # Fallback to principalId or arn
            if not creator_name:
                creator_name = user_identity.get('principalId') or user_identity.get('arn', '').split('/')[-1]
        ...
        event_info = {
            ...
            'user_identity': detail.get('userIdentity', {}),
            'creator_name': creator_name,  # User who performed the action
    ...
    def _recursive_extract(self, obj: Any, resources: Dict[str, list], depth: int = 0, max_depth: int = 10):
        ...
        # CloudTrail metadata fields to exclude from resource extraction
        excluded_fields = {
            'requestid', 'eventid', 'receipthandle', 'messageattributevalue',
            'responsemetadata', 'requestparameters', 'responseelements',
            'eventversion', 'eventtime', 'eventcategory', 'readonly',
            'managementevent', 'recipientaccountid', 'eventtype',
        ...
        if isinstance(obj, dict):
            for key, value in obj.items():
                ...
                # Skip excluded CloudTrail metadata fields
