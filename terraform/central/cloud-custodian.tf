...
variable "mailer_from_address" {
  ...
  type        = string
  default     = "c7n@central.ctkube.com"
}
...
# ============================================================================
# SES Domain Identity and DKIM Configuration (us-east-1 only)
# ============================================================================

# SES Domain Identity for central.ctkube.com
resource "aws_ses_domain_identity" "main" {
  domain = "central.ctkube.com"
}

# Data source for Route53 zone
data "aws_route53_zone" "main" {
  name = "central.ctkube.com"
}

# Route53 TXT record for SES domain verification
# This resource implements verification workflow for SES domain identity
...
# Creation of this record represents successful verification of an SES domain identity
...
# DKIM tokens generated by SES for email authentication
resource "aws_ses_domain_dkim" "main" {
  domain = aws_ses_domain_identity.main.domain
}

# Route53 CNAME records for DKIM verification (3 records required)
resource "aws_route53_record" "amazonses_dkim_verification_record" {
  count   = 3
  zone_id = data.aws_route53_zone.main.zone_id
  name    = "${element(aws_ses_domain_dkim.main.dkim_tokens, count.index)}._domainkey.central.ctkube.com"
  type    = "CNAME"
  ttl     = "600"
...
# AWS handles redirection to different regions internally
# Reference: https://docs.aws.amazon.com/ses/latest/DeveloperGuide/regions.html#region-mail-from
...
# Route53 MX record for FROM Domain
# Some legacy recipient mail servers validate that the email domain has a valid MX record
resource "aws_route53_record" "ses_domain_from_mx" {
  zone_id = data.aws_route53_zone.main.zone_id
  name    = "central.ctkube.com"
  type    = "MX"
  ttl     = "600"
...
# Route53 TXT record for DMARC (Domain-based Message Authentication, Reporting & Conformance)
# DMARC builds on SPF and DKIM to enable senders and receivers to collaborate on mail authentication
# Reference: https://dmarc.org/overview/
resource "aws_route53_record" "ses_domain_dmarc_txt" {
  zone_id = data.aws_route53_zone.main.zone_id
  name    = "_DMARC.${aws_ses_domain_identity.main.domain}"
  type    = "TXT"
  ttl     = "600"
  records = ["v=DMARC1; p=none; sp=none; fo=1; rua=mailto:dmarc_rua@emaildefense.proofpoint.com; ruf=mailto:dmarc_ruf@emaildefense.proofpoint.com"]
}

# SES domain identity policy for cross-account access
...
output "ses_properties" {
  value = {
    domain_identity_arn                = aws_ses_domain_identity.main.arn
    domain_identity_verification_token = aws_ses_domain_identity.main.verification_token
    domain_identity_dkim_tokens        = [aws_ses_domain_dkim.main.dkim_tokens]
  }
}

output "ses_domain_verification_token" {
  description = "SES domain verification TXT record value - Add this to central.ctkube.com DNS"
  value       = aws_ses_domain_identity.main.verification_token
}

output "ses_dkim_tokens" {
  description = "DKIM CNAME record values - Add these 3 records to central.ctkube.com DNS"
  value = [
    for token in aws_ses_domain_dkim.main.dkim_tokens :
    "${token}._domainkey.central.ctkube.com CNAME ${token}.dkim.amazonses.com"
  ]
}
